...
from TechnicalAnalysisAutomation.perceptually_important import find_pips
from TechnicalAnalysisAutomation.rolling_window import rw_top, rw_bottom
from TechnicalAnalysisAutomation.trendline_automation import fit_trendlines_single
...
def find_flags_pennants_trendline(data: np.array,  weight: int, dataFrame):
        ...
        if pending_bear is not None:
            if check_bear_pattern_trendline(pending_bear, data, i, order):
                if pending_bear.pennant:
                    ...
                    bear_flags.append(pending_bear)
                    dataFrame.loc[pending_bear.conf_x, "pennants"] = -weight
                pending_bear = None
        ...
        if pending_bull is not None:
            if check_bull_pattern_trendline(pending_bull, data, i, order):
                if pending_bull.pennant:
                    ...
                    bull_flags.append(pending_bull)
                    dataFrame.loc[pending_bull.conf_x, "pennants"] = weight

                pending_bull = None
    return dataFrame["pennants"]
